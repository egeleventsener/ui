    ...
    for(;;){
        ...
        if(!strncmp(buffer,"send_file",9)){
            ...
            if(!strncasecmp(mode,"local",5)){
                ...
                FILE *out=fopen(dest,"wb"); if(!out){ perror("open dst"); fclose(in); continue; }
                char bufc[BUF_SIZE]; size_t r;
                while((r=fread(bufc,1,sizeof(bufc),in))>0) fwrite(bufc,1,r,out);
            ...
            if(dest[0]!='\0'){
                char scd[PATH_MAX+8]; int m=snprintf(scd,sizeof(scd),"scd %s\n",dest);
                if(m>0){ if(send_all(sock, scd, (size_t)m)<0){ perror("send scd"); fclose(fp); continue; } }
            }
            ...
            if (send_file_with_size(fp, sock, src) < 0) perror("send file");
            fclose(fp);
        ...
        if(n>0){ reply[n]='\0'; printf("Server: %s", reply); }
