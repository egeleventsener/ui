...
static int secure_cd(const char *target){
    ...
    if(target[0] == '/'){
        ...
        if(len + 1 < sizeof(tmp)){ tmp[len] = '/'; tmp[len+1] = '\0'; }
...
static void handle_command(int client, char *cmdline){
    if(strcmp(cmdline, "spwd") == 0){
        ...
        } else send_str(client, "pwd fail\n");
        return;
    }
    if(starts_with(cmdline, "scd ")){
    ...
    if(starts_with(cmdline, "srm ")){
        const char *path = cmdline + 4;
        char canon[PATH_MAX];
        if(!realpath(path, canon) || !secure_path_in_base(canon)){ send_str(client,"Failed to delete\n"); return; }
        if(delete_directory(canon)==0) send_str(client,"Deleted\n");
        else send_str(client,"Failed to delete\n");
        return;
    }
    if(starts_with(cmdline, "srename ")){
        ...
        if(oldn && newn){
            ...
            if(realpath(oldn,c1) && realpath(newn,c2) && secure_path_in_base(c1) && secure_path_in_base(c2) && rename(c1,c2)==0)
                send_str(client,"Renamed\n");
            else send_str(client,"Rename failed\n");
        } else send_str(client,"Invalid rename command\n");
        return;
    }
    if(strcmp(cmdline, "write_file") == 0){
        char fname[PATH_MAX];
        if(recv_line(client, fname, sizeof(fname)) < 0 || fname[0]=='\0'){ send_str(client,"filename error\n"); return; }
        char sizeln[128];
...
int main(void){
    ...
    { char canon[PATH_MAX];
      if(realpath(START_DIR, canon)){ strncpy(BASE_DIR, canon, sizeof(BASE_DIR)-1); BASE_DIR[sizeof(BASE_DIR)-1]='\0'; }
      else { strncpy(BASE_DIR, START_DIR, sizeof(BASE_DIR)-1); BASE_DIR[sizeof(BASE_DIR)-1]='\0'; } }

    int srv = socket(AF_INET, SOCK_STREAM, 0);
    if(srv<0){ perror("socket"); return 1; }
    int opt=1; setsockopt(srv, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
    struct sockaddr_in addr = {0};
    addr.sin_family = AF_INET; addr.sin_port = htons(5000); addr.sin_addr.s_addr = htonl(INADDR_ANY);
